...
class BirdViewCameraCityScapesSensor(CameraCityScapesSensor):
    ...
    def default(
        cls, hero: carla.ActorBlueprint, *args, **kwargs  # pylint: disable=no-member
    ) -> "BirdViewCameraCityScapesSensor":
        """Returns the default sensor."""
        return cls(
            hero=hero,
...
class CollisionSensor(simulator.Sensor):
    ...
    def observation_space(self, *args: Any, **kwargs: Any) -> gym.spaces.Box:
        ...
        return gym.spaces.Discrete(
            n=4
        )  # {0: no-collision, 1: vehicle, 2: pedestrian, 3: other}
...
class GameStateSensor(simulator.Sensor):
    ...
    def get_observation(self, **kwargs) -> np.ndarray:
        ...
        pedestrians_surface = gutil.get_pedestrians_surface(world=world, **self.config)
        green_surface, yellow_surface, red_surface = gutil.get_traffic_lights_surface(
            world=world, **self.config
        )
        hero_surface = gutil.get_hero_surface(
            world=world, hero=self.hero, **self.config
        )
...
class CARLASimulator(simulator.Simulator):
    ...
    def __init__(
        ...
        spawn_point: Optional[
            Union[int, carla.Location]
        ] = None,  # pylint: disable=no-member
    ...
    def close(self) -> None:
        ...
        logging.debug(
            "Closes the CARLA server with process PID {}".format(self._server.pid)
        )
