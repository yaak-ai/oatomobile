...
def run_agent(agent, ckpt, town, num_vehicles, num_pedestrians,
              length, down_sampling, save_path=None, interactive=False):
  ...
  model = ImitativeModel(output_shape=[down_sampling, 2])
...
if __name__ == '__main__':
  ...
  parser.add_argument('-s', '--sampling', dest="sampling", type=int,
                      default=4, help="Downsampling for trajactory")
  parser.add_argument('-l', '--length', dest="length", type=int,
                      default=80, help="Length of generated trajactory")
  ...
  run_agent(args.agent, args.weights, args.town, args.n_vehicles,
            args.n_pedestrians, args.length, args.sampling,
            args.vid, args.interactive)
