...
class Env(gym.Env):
    ...
    def __init__(
        self, sim_fn: Callable[..., Simulator], *args: Any, **kwargs: Any
    ) -> None:
...
class FiniteHorizonWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env, *, max_episode_steps: int) -> None:
        ...
        self._pbar = tqdm.tqdm(total=self._max_episode_steps, ascii=True)
...
class MonitorWrapper(gym.Wrapper):
    ...
    def __init__(
        self, env: gym.Env, *, output_fname: str, downsample_factor: int = 1
    ) -> None:
