...
class SetPointAgent(oatomobile.Agent):
    ...
    def __init__(
        ...
        *,
        ...
        logging.debug("Lateral PID controller config {}".format(lateral_control_dict))
        longitudinal_control_dict = longitudinal_control_dict.copy()
        longitudinal_control_dict.update({"dt": dt})
        logging.debug(
            "Longitudinal PID controller config {}".format(longitudinal_control_dict)
        )
        ...
        self._fixed_delta_seconds_between_setpoints = (
            fixed_delta_seconds_between_setpoints or dt
        )
    ...
    @abc.abstractmethod
    def __call__(
        self, observation: oatomobile.Observations, *args, **kwargs
    ) -> np.ndarray:
    ...
    def act(
        ...
        if (
            self._setpoints_buffer is None
            or self._steps_counter % self._replan_every_steps == 0
        ):
            # Get agent predictions.
            predicted_plan_ego = self(
                copy.deepcopy(observation), *args, **kwargs
            )  # [T, 3]
