...
def setup(
    ...
    carla.Client, carla.World, int, subprocess.Popen
...
def carla_rgb_image_to_ndarray(
    image: carla.Image,
) -> np.ndarray:  # pylint: disable=no-member
...
def spawn_hero(
    ...
    logging.debug(
        "Spawns hero actor at {}".format(carla_xyz_to_ndarray(spawn_point.location))
    )
...
def spawn_rss(
    ...
    Args:
      hero: The agent to attach the RSS sensor on.
    ...
    bl = world.get_blueprint_library()
    # Configure blueprint.
    rss_bp = bl.find("sensor.other.rss")
    logging.debug("Spawns a lane invasion sensor")
    return world.spawn_actor(
        rss_bp,
...
def get_actors(
    ...
    carla.Vehicle,
    Sequence[Optional[carla.Vehicle]],  # pylint: disable=no-member
    Sequence[Optional[carla.Walker]],
]:  # pylint: disable=no-member
...
def ndarray_to_vector3d(
    array: np.ndarray,
) -> carla.Vector3D:  # pylint: disable=no-member
...
def ndarrays_to_vehicle_measurements(
    ...
    return dict(
        control=carla.VehicleControl(
            *list(map(float, observation["control"]))
        ),  # pylint: disable=no-member
        ...
        traffic_light_state=carla.TrafficLightState.values[
            int(observation["traffic_light_state"])  # pylint: disable=no-member
        ],
    )
