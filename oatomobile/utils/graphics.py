...
def make_dashboard(
    ...
    **observations: np.ndarray
    ...
    if "is_at_traffic_light" in observations and "traffic_light_state" in observations:
        if (
            observations["is_at_traffic_light"] == 1
            and observations["traffic_light_state"] == 0
        ):
...
def draw_settings(
    ...
    return world_offset, pygame.Surface(
...
def get_lane_boundaries_surface(
    ...
    def get_lane_markings(lane_marking_type, waypoints, sign):
        ...
        if (
            lane_marking_type == carla.LaneMarkingType.Broken
            or (  # pylint: disable=no-member
            ...
            if (
                ...
                lane_marking_type == carla.LaneMarkingType.BrokenSolid
            ):  # pylint: disable=no-member
                return [
                    (
                        carla.LaneMarkingType.Solid,
                ...
                lane_marking_type == carla.LaneMarkingType.BrokenBroken
            ):  # pylint: disable=no-member
                return [
                    (
                        carla.LaneMarkingType.Broken,
                ...
                lane_marking_type == carla.LaneMarkingType.SolidSolid
            ):  # pylint: disable=no-member
                return [
                    (
                        carla.LaneMarkingType.Solid,
    ...
    def draw_lane_marking_single_side(surface, waypoints, sign):
        ...
        for markings in markings_list:
            if markings[0] == carla.LaneMarkingType.Solid:  # pylint: disable=no-member
                ...
                markings[0] == carla.LaneMarkingType.Broken
            ):  # pylint: disable=no-member
                draw_broken_line(
                    surface,
                    COLORS["WHITE"],
...
def get_pedestrians_surface(
    ...
    pedestrians = [
        actor for actor in world.get_actors() if "walker.pedestrian" in actor.type_id
    ]
...
def actor_to_corners(
    actor: carla.Actor,
) -> Sequence[Tuple[float, float]]:  # pylint: disable=no-member
